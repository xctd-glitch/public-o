...
document.addEventListener('alpine:init', () => {
    Alpine.data('dash', () => ({
        ...
        // Postback Methods
        async loadPostbacks() {
            this.postbackFilters.loading = true;

            try {
                const params = new URLSearchParams({
                    start_date: this.postbackFilters.start_date,
                    end_date: this.postbackFilters.end_date
                });

                const response = await fetch('/postback-data.php?' + params.toString(), {
                    method: 'GET',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                ...
                if (!result.ok) {
                    throw new Error(result.error || 'Failed to fetch postback data');
                }
                ...
                this.setFlash('Failed to load postback data: ' + error.message, 'error');
            } finally {
                this.postbackFilters.loading = false;
            }
        },
        ...
        copyPostbackUrl() {
            ...
            const url = protocol + '//' + host + '/postback.php?payout={payout}&country={country}&traffic_type={traffic_type}';

            navigator.clipboard.writeText(url).then(() => {
                this.setFlash('Postback URL copied to clipboard!', 'info');
            }).catch(() => {
        ...
        getChartHeight(conversions) {
            if (conversions === 0) return 0;
            const maxConversions = Math.max(...this.postbackChartData.map(d => d.conversions), 1);
            return (conversions / maxConversions) * 100;
        },
